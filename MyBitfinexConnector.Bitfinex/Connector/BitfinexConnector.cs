...
public class BitfinexConnector : ITestConnector
{
    ...
    private readonly ConcurrentDictionary<string, long> _lastTradeOffsetByPair = new();
    ...
    {
        var lastId = _lastTradeOffsetByPair.GetValueOrDefault(trade.Pair, long.MinValue);
        if (trade.Id <= lastId)
            return;

        _lastTradeOffsetByPair[trade.Pair] = trade.Id;
