...
namespace MyBitfinexConnector.Bitfinex.Tests.Integration.Connector;

public class BitfinexConnector_Tests : IAsyncLifetime
{
    private static readonly string Pair = "BTCUSD";
    ...
    public async Task GetTradesAsync_LimitIsWork()
    {
        ...
        var trades = await _connector.GetTradesAsync(Pair, limit);
    ...
    public async Task GetTradesAsync_LimitFromToIsWork()
    {
        ...
        var trades = await _connector.GetTradesAsync(Pair, limit, from, to);
    ...
    public async Task GetCandlesAsync_PeriodIsWork()
    {
        ...
        foreach (var candles in await Task.WhenAll(candlesFromSec)) candles.Should().HaveCount(1);

        var candlesFromString = CandlePeriodMapper.SecondsToString.Values
            .Select(period => _connector.GetCandlesAsync(Pair, period, limit: 1));

        foreach (var candles in await Task.WhenAll(candlesFromString)) candles.Should().HaveCount(1);
    }
    ...
    public async Task SubscribeCandles_ShouldReceiveCandle()
    {
        ...
        void Handle(Candle candle)
        {
            tcs.TrySetResult(candle);
        }
    }
